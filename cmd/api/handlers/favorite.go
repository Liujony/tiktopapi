...
func FavoriteAction(ctx context.Context, c *app.RequestContext) {
	...
	if err != nil {
		SendResponse(c, &api.DouyinFavoriteActionResponse{StatusCode: int32(1), StatusMsg: "Invalid UserId"})
		return
	}

	vid, err := strconv.Atoi(video_id)
	if err != nil {
		SendResponse(c, &api.DouyinFavoriteActionResponse{StatusCode: int32(2), StatusMsg: "Invalid VideoId"})
		return
	}
	act, err := strconv.Atoi(action_type)
	if err != nil {
		// SendResponse(c, pack.BuildFavoriteActionResp(errno.ErrBind))
		SendResponse(c, &api.DouyinFavoriteActionResponse{StatusCode: int32(3), StatusMsg: "Invalid ActionType"})
		return
	}
	...
	resp, err := rpc.FavoriteAction(ctx, &api.DouyinFavoriteActionRequest{
	...
	if err != nil {
		...
		SendResponse(c, &api.DouyinFavoriteActionResponse{StatusCode: int32(4), StatusMsg: "RPC Resp Failed"})
		return
	}
	SendResponse(c, resp)
}
